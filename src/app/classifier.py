...
def classify_source_group(source, category_patterns, config):
    ...
    # 如果类型存在且不是小说（0），先加入类型标签
    # 对于非小说类型，添加类型标签
    if type_label and source.book_source_type != 0:
        matched.append(type_label)
    # 对于小说类型，根据配置决定是否添加默认标签
    elif (
        type_label
        and source.book_source_type == 0
        and config.classify.use_novel_default_label
    ):
        matched.append(type_label)

    # 收集所有需要匹配的文本：分组、名称（可选）、注释（可选）
    texts_to_match = [source.book_source_group or ""]

    # 如果启用名称分类，添加名称到匹配文本
    if config.classify.use_name_for_classify:
        texts_to_match.append(source.book_source_name or "")

    # 如果启用注释分类，添加注释到匹配文本
    if config.classify.use_comment_for_classify:
        texts_to_match.append(source.book_source_comment or "")

    # 合并所有文本进行匹配
    combined_text = " ".join(texts_to_match)

    # 遍历分类规则，匹配所有收集到的文本
    for group_name, pattern in category_patterns.items():
        if pattern.search(combined_text) and group_name not in matched:
            matched.append(group_name)

    # 如果没有匹配到标签，但类型是小说 → 根据配置决定是否添加默认标签
    if (
        not matched
        and source.book_source_type == 0
        and config.classify.use_novel_default_label
